...
class BertEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        ret = {
            "aggregate": aggregate, # 这是通过对所有token的特征进行加权平均得到的 [batch_size, 768]
            "embedded": embedded, # 每个token位置的详细特征 [batch_size, sequence_length, 768]
            "masks": mask, # 用于标识哪些是实际token，哪些是padding [batch_size, sequence_length]
            "hidden": encoded_layers[-1] # BERT最后一层的直接输出 [batch_size, sequence_length, 768]
        }
        return ret
