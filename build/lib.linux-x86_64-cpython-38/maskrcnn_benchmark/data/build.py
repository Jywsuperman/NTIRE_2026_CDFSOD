...
def build_dataset(cfg, dataset_list, transforms, dataset_catalog, is_train=True, class_concat=False, extra_args={}):
    ...
    for dataset_id, dataset_name in enumerate(dataset_list, 1):
        ...
        # print(data["factory"]) # CocoGrounding
        # print("哈哈")
        # sys.exit()
...
def make_data_sampler(dataset, shuffle, distributed, num_replicas=None, rank=None, use_random_seed=True, seed=42):
    if distributed:
        return samplers.DistributedSampler(dataset, shuffle=shuffle, num_replicas=num_replicas, rank=rank,
                                           use_random=use_random_seed)

    if shuffle:
        # # 为RandomSampler提供确定的生成器
        # # 这确保每次运行时采样顺序一致
...
def make_data_loader(cfg, is_train=True, is_distributed=False, num_replicas=None, rank=None, start_iter=0):
    ...
    if len(cfg.DATASETS.REGISTER) > 0: # 注册数据集  这里构建训练数据集和测试数据集
    ...
    data_loaders = [] 
    for di, dataset in enumerate(datasets): # 迭代次数 = (epoch数 × 样本数) ÷ batch_size
        if is_train and cfg.SOLVER.MAX_EPOCH > 0:
            num_iters = cfg.SOLVER.MAX_EPOCH * len(dataset) // cfg.SOLVER.IMS_PER_BATCH

            # NEUDET 200 60 4 3000 （6个类，每个类5个样本）
            # UODD 200 20 4 1000 （3个类，一共是13张图片，一共有15个框）
            # Clipart1k 200 184 4 9200 （20个类，一共是92张图片，一共有100个框）
            # FISH 200 10 5 500 (1个类)
            # ArTaxOr 200 70 4 3500 （7个类）
            # DIOR 200 172 4 8400 (20个类，一共是86张图片，一共有100个框)
        ...
        def worker_init_fn(worker_id):
            ...
            # 为每个worker设置不同但确定的种子
            worker_seed = cfg.DATASETS.SHUFFLE_SEED + worker_id
            np.random.seed(worker_seed)
            random.seed(worker_seed)
            torch.manual_seed(worker_seed)
            # 设置Python的哈希种子可以减少字典和集合等结构的随机性
            os.environ['PYTHONHASHSEED'] = str(worker_seed)

            # worker_seed = cfg.DATASETS.SHUFFLE_SEED * cfg.num_gpus + worker_id + dist.get_rank()
            # random.seed(worker_seed)
            # np.random.seed(worker_seed)
            # torch.manual_seed(worker_seed)
        ...
        data_loader = torch.utils.data.DataLoader(
            ...
            generator=torch.Generator().manual_seed(cfg.DATASETS.SHUFFLE_SEED),  # 添加生成器
            persistent_workers=True if num_workers > 0 else False  # 减少worker启动的随机性
        )
