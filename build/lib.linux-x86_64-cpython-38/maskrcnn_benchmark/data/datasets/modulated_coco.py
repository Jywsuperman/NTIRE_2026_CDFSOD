...
from maskrcnn_benchmark.data.datasets.coco import CocoDetection  # 导入我们的自定义实现

# class CocoGrounding(torchvision.datasets.CocoDetection): # 改动1，需要额外处理字符串id
class CocoGrounding(CocoDetection):
    def __init__(self,
                 img_folder,
                 ann_file,
                 transforms,
        ...
        self.ids = sorted(self.ids) # 排序图片ID
        ...
        if few_shot:
            ...
            ##########################################################
            # print(cats_freq) # [5, 5, 5]
            # print(len(self.coco.cats.keys())) # 3
            # print("哈哈")
            # sys.exit()
            min_cat_id = min(self.coco.cats.keys())
            # 根据最小类别 ID 决定 cats_freq 的长度和索引方式
            if min_cat_id == 0:
                # 如果从 0 开始，则数组大小为 max+1
                cats_freq = [few_shot] * (max(self.coco.cats.keys()) + 1)
                index_offset = 0
            else:
                # 如果从 1 开始，保持原有逻辑
                cats_freq = [few_shot] * max(self.coco.cats.keys())
                index_offset = 1
            ...
            for img_id in self.ids:
                if isinstance(img_id, str):
                    ...
                    ann_ids = self.coco.getAnnIds(imgIds=img_id, iscrowd=None)
                anno = self.coco.loadAnns(ann_ids)
                cat = set([ann['category_id'] for ann in anno])
                
                # 使用 index_offset 来正确索引
                is_needed = sum([cats_freq[c - index_offset] > 0 for c in cat])
                if is_needed:
                    ids.append(img_id)
                    for c in cat:
                        cats_freq[c - index_offset] -= 1
            ##########################################################
        ...
        has_zero_category = 0 in self.coco.getCatIds()

        # 根据是否有类别ID为0来决定映射方式
        if has_zero_category:
        ...
        # self.json_category_id_to_contiguous_id = { # 类别映射
        #     v: i + 1 for i, v in enumerate(self.coco.getCatIds())
        # }
        
        # print(self.coco.getCatIds()) [0, 1, 2]
        # print("哈哈1")
        # print(self.json_category_id_to_contiguous_id) {0: 1, 1: 2, 2: 3}
        # sys.exit()
    ...
    def categories(self, no_background=True):
        ...
        # Determine the minimum category ID in the dataset
        min_cat_id = min(self.coco.cats.keys())
        ...
        for index, i in enumerate(categories):
            ...
            if not no_background or (i["name"] != "__background__"):
...
class CocoDetection(data.Dataset):
    ...
    def __init__(self, root, annFile, transform=None, target_transform=None):
        ...
        self.root = root # 图像根目录
        self.coco = COCO(annFile) # 加载COCO格式的标注文件
        self.ids = list(self.coco.imgs.keys()) # 获取所有图片ID
    ...
    def __getitem__(self, index, return_meta=False):
        ...
        if isinstance(img_id, str):# 和torch自带的最大不同是，把字符串的转换为了列表
            img_id = [img_id]

        ann_ids = coco.getAnnIds(imgIds=img_id) # 返回与该图片相关的所有标注ID
        target = coco.loadAnns(ann_ids) # 加载所有具体的标注信息，图片+标注ID

        meta = coco.loadImgs(img_id)[0] # 获取图像信息
        path = meta['file_name']
        img = pil_loader(os.path.join(self.root, path)) # 加载图像
...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image, target, ignore_box_screen=False, box_format="xywh"):
        ...
        if isinstance(image_id, str): # 改动2，需要字符串通过哈希转换为普通的数字，用于追踪
            # 使用字符串的哈希值作为ID，确保是32位整数
            image_id = hash(image_id) % (2**32)
