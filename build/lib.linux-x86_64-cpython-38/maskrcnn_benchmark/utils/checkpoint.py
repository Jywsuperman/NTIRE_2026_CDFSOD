...
class Checkpointer(object):
    ...
    def save(self, name, **kwargs):
        ...
        torch.save(data, save_file) # 把保存给开启
    ...
    def load(self, f=None, force=False, keyword="model", skip_optimizer =False):
        ...
        if resume and not skip_optimizer:
            if "optimizer" in checkpoint and self.optimizer:
                ...
                # 获取预训练优化器的状态
                pretrained_optimizer_state = checkpoint.pop("optimizer")
                ...
                for i, group in enumerate(current_param_groups):
                    for param in group['params']:
                        param_id = id(param)
                        if param_id in pretrained_optimizer_state['state']:
                            new_optimizer_state['state'][param_id] = pretrained_optimizer_state['state'][param_id]
