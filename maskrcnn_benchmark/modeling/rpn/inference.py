...
class ATSSPostProcessor(torch.nn.Module):
    ...
    def forward_for_single_feature_map(self, box_regression, centerness, anchors,
        ...
        N, _, H, W = box_regression.shape # 特征图的尺寸

        A = box_regression.size(1) // 4 # 每个位置anchor的数量
        ...
        if box_cls is not None:
            ...
            box_cls = permute_and_flatten(box_cls, N, A, C, H, W) # 将预测的偏移量reshape成与anchor对应的格式
            box_cls = box_cls.sigmoid()
        ...
        candidate_inds = box_cls > self.pre_nms_thresh 
        # 1、根据置信度阈值pre_nms_thresh筛选出候选框
        # 限制每张图片保留的候选框数量不超过pre_nms_top_n
        pre_nms_top_n = candidate_inds.reshape(N, -1).sum(1)
        pre_nms_top_n = pre_nms_top_n.clamp(max=self.pre_nms_top_n)
        ...
        box_cls = box_cls * centerness[:, :, None] # 2、将分类得分与中心度得分相乘，得到最终的置信度分数
        ...
        for per_box_cls, per_box_regression, per_pre_nms_top_n, per_candidate_inds, per_anchors \
            ...
            per_box_cls, top_k_indices = per_box_cls.topk(per_pre_nms_top_n, sorted=False) # 3、对每个类别选择置信度最高的K个框

            per_candidate_nonzeros = per_candidate_inds.nonzero()[top_k_indices, :]

            per_box_loc = per_candidate_nonzeros[:, 0] # per_box_loc表示在特征图上的位置索引
            per_class = per_candidate_nonzeros[:, 1] + 1

            # print(per_class)

            detections = self.box_coder.decode( # 4、将预测的偏移量解码为实际的边界框坐标
                # 使用位置索引获取对应的anchor和预测的偏移量
                per_box_regression[per_box_loc, :].view(-1, 4),  # 这个位置的预测偏移量
                per_anchors.bbox[per_box_loc, :].view(-1, 4) # 这个位置的anchor
            )

            boxlist = BoxList(detections, per_anchors.size, mode="xyxy")
            boxlist.add_field("labels", per_class)
            boxlist.add_field("scores", torch.sqrt(per_box_cls))
            boxlist = boxlist.clip_to_image(remove_empty=False) # 将框裁剪到图像边界内
            boxlist = remove_small_boxes(boxlist, self.min_size) # 移除太小的框
            results.append(boxlist)
    ...
    def forward(self, box_regression, centerness, anchors,
        ...
        boxlists = list(zip(*sampled_boxes))
        boxlists = [cat_boxlist(boxlist) for boxlist in boxlists] # 5、将不同特征层级的预测结果合并
    ...
    def select_over_all_levels(self, boxlists):
        ...
        for i in range(num_images):
            # multiclass nms
            result = boxlist_ml_nms(boxlists[i], self.nms_thresh) # 6、使用非极大值抑制(NMS)去除重叠的框
            number_of_detections = len(result)

            # Limit to max_per_image detections **over all classes**
            if number_of_detections > self.fpn_post_nms_top_n > 0: # 7、限制每张图片最终保留的检测框数量不超过fpn_post_nms_top_n
                cls_scores = result.get_field("scores")
                image_thresh, _ = torch.kthvalue(
                ...
                keep = cls_scores >= image_thresh.item() # 根据置信度分数选择最可靠的框
                keep = torch.nonzero(keep).squeeze(1)
                result = result[keep]
            results.append(result)
        return results
