...
def build_retinanet_swint_fpn_backbone(cfg):
    ...
    fpn = fpn_module.FPN( # 每个尺度的特征图负责检测相应大小范围的目标
        in_channels_list=[
            0,
        ...
        top_blocks=fpn_module.LastLevelP6P7(in_channels_p6p7, out_channels), # 添加了P6和P7层，用于检测更大尺度的目标
        drop_block=DropBlock2D(cfg.MODEL.FPN.DROP_PROB, cfg.MODEL.FPN.DROP_SIZE) if cfg.MODEL.FPN.DROP_BLOCK else None, # 用于正则化的特征dropout
        use_spp=cfg.MODEL.FPN.USE_SPP, # 空间金字塔池化(Spatial Pyramid Pooling)
        use_pan=cfg.MODEL.FPN.USE_PAN, # 特征金字塔注意力网络(Path Aggregation Network)
        return_swint_feature_before_fusion=cfg.MODEL.FPN.RETURN_SWINT_FEATURE_BEFORE_FUSION
    )
    if cfg.MODEL.FPN.USE_DYHEAD:
        dyhead = DyHead(cfg, out_channels) # 增强不同尺度特征之间的信息交互
        model = nn.Sequential(OrderedDict([("body", body), ("fpn", fpn), ("dyhead", dyhead)]))
    else:
        model = nn.Sequential(OrderedDict([("body", body), ("fpn", fpn)]))
    # 基础特征提取，构建多尺度特征表示处理不同大小的目标，进一步优化特征
    return model
