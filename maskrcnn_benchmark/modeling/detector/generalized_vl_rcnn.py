...
class GeneralizedVLRCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.LANGUAGE_BACKBONE.TOKENIZER_TYPE == "clip":
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(cfg.MODEL.LANGUAGE_BACKBONE.TOKENIZER_TYPE, local_files_only=True)
            # bert-base-uncased
        ...
        if self.cfg.MODEL.USE_Prototype:
    ...
    def forward(self, 
        ...
        if self.cfg.GLIPKNOW.PARALLEL_LANGUAGE_INPUT:
            ...
            if captions is not None:
                ...
                # 处理背景文本
                background_text_features = None
                if self.cfg.MODEL.USE_BACKGROUND:
                    background_texts = self.cfg.MODEL.BACKGROUND_TEXT  # 从配置中获取背景文本
                    background_tokenized = self.tokenizer.batch_encode_plus(
                        background_texts,
                        max_length=self.cfg.MODEL.LANGUAGE_BACKBONE.MAX_QUERY_LEN,
                        padding='max_length' if self.cfg.MODEL.LANGUAGE_BACKBONE.PAD_MAX else "longest",
                        return_special_tokens_mask=True,
                    ...
                    background_tokenizer_input = {
                        "input_ids": background_tokenized.input_ids,
                        "attention_mask": background_tokenized.attention_mask
                    }
                    
                    if self.cfg.MODEL.LANGUAGE_BACKBONE.FREEZE:
                        with torch.no_grad():
                            background_features = self.language_backbone(background_tokenizer_input)
                    else:
                        background_features = self.language_backbone(background_tokenizer_input)
                    # background_text_features = background_features['aggregate']
                    background_text_features = background_features


                if self.use_mlm_loss: # 可选的MLM（掩码语言模型）处理
                ...
                if self.cfg.MODEL.LANGUAGE_BACKBONE.FREEZE: # 通过语言backbone提取特征
                    with torch.no_grad():
        ...
        if self.force_boxes: # 两种模式：强制使用目标框或正常RPN
            proposals = []
            for t in targets: # 直接使用ground truth boxes
            ...
            if self.cfg.MODEL.RPN.RETURN_FUSED_FEATURES:
                _, proposal_losses, fused_visual_features = self.rpn( # 通过RPN生成proposals+特征
            ...
            proposals, proposal_losses, fused_visual_features = self.rpn(images, visual_features, targets, language_dict_features, positive_map,
                                              captions, swint_feature_c4, background_text_features)
        if self.roi_heads:
            if self.cfg.MODEL.ROI_MASK_HEAD.PREDICTOR.startswith("VL"): # 处理VL mask head的特殊情况
                if self.training:
                    # "Only support VL mask head right now!!"
                    # # 确保targets和positive_map匹配
                    ...
                    targets[0].add_field("positive_map", positive_map)  # 添加positive_map到targets
            # TODO: make sure that this use of language_dict_features is correct!! Its content should be changed in self.rpn
            if self.cfg.MODEL.RPN.RETURN_FUSED_FEATURES: # 根据是否使用融合特征选择不同处理
        ...
        if self.training: # 训练模式：返回损失
            losses = {} # roihead的损失+grounding的损失（toekn，对比，点积）
            losses.update(detector_losses)
            losses.update(proposal_losses)
            return losses 

        return result  # 测试模式：返回检测结果
