...
def create_queries_and_maps_from_dataset(dataset, cfg):
    ...
    for cat_id, cat_info in dataset.coco.cats.items():
        original_categories[cat_id] = cat_info["name"]

    # 按照类别ID排序
    labels = []
    label_list = []
    keys = list(original_categories.keys())
    keys.sort()  # 按照ID排序 (0, 1, 2, ...)
    for i in keys:
        labels.append(i)
        label_list.append(original_categories[i])

    # print("类别ID:", labels)
    # print("类别名称:", label_list)
    # sys.exit()
    ######################################
...
def inference(
    ...
    if cfg.GLIPKNOW.PARALLEL_LANGUAGE_INPUT:
        ...
        # print("all_queries 类型:", type(all_queries))
        # sys.exit()
        # print("all_queries 内容:", all_queries)
        # print("all_queries 长度:", len(all_queries))

        # print("\nall_positive_map_label_to_token 类型:", type(all_positive_map_label_to_token))
        # print("all_positive_map_label_to_token 内容:", all_positive_map_label_to_token)
        # print("all_positive_map_label_to_token 长度:", len(all_positive_map_label_to_token))
        # print("哈哈")
        # sys.exit()
    elif task == "grounding":
        all_queries = [None]
    ...
    for i, batch in enumerate(_iterator):
        ...
        if i == cfg.TEST.SUBSET:
            break
        images, targets, image_ids, *_ = batch # 推理阶段批次一般是1

        # print(image_ids)
        # print("哈哈")

        # print("图像张量形状:", images.tensors.shape) torch.Size([1, 3, 768, 1344])
        # print("原始图像尺寸:", images.image_sizes)  [torch.Size([750, 1333])]
        # print("哈哈")

        # print("targets 类型:", type(targets))
        # print("targets 内容:", targets)
        # print("targets 长度:", len(targets))

        # print("\nimage_ids 类型:", type(image_ids))
        # print("image_ids 内容:", image_ids)
        # print("image_ids 长度:", len(image_ids))
        # sys.exit()
        ...
        with torch.no_grad():
            if cfg.TEST.USE_MULTISCALE:
                ...
                for query_i in range(query_time):
                    ...
                    if task == "detection":
                        captions = [all_queries[query_i] for ii in range(len(targets))] # 有可能这个文本，是对应图片中的所有target的
                        positive_map_label_to_token = all_positive_map_label_to_token[query_i]
                    elif task == "grounding":
                        ...
                        # 大量的训练数据
                        # 精心设计的 positive_map_label_to_token 映射
                    
                    # print(captions) ['seaurchin. scallop']
                    # print(positive_map_label_to_token) defaultdict(<class 'list'>, {2: [1, 2, 3], 3: [5, 6, 7]})
                    # print("哈哈2")
                    # sys.exit()

                    output = model(images, captions=captions, positive_map=positive_map_label_to_token)
                    # output = model(images)  # 只传入图像
                    output = [o.to(cpu_device) for o in output]
                    ...
                    if "flickr" in cfg.DATASETS.TEST[0]:
                        ...
                        all_output.append(output)  # 每个q都会进行一次输出，所以最终是1100，每个图片（506）两个
        ...
        if visualizer is not None:
            ...
            # sys.exit()
            # 创建保存目录
            os.makedirs(os.path.join(output_folder, "visual"), exist_ok=True)
            imshow(result, os.path.join(output_folder, "visual", "img_{}.jpg".format(batch_image_index)))
