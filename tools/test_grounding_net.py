...
def main():
    ...
    if args.weight:
        ...
        # # 从配置文件名称中提取数据集名称
        # dataset_name = os.path.basename(args.task_config).split('.')[0]  # 例如从 'FISH.yaml' 提取 'FISH'
        
        # # 构建新的日志路径，在eval后面加上数据集名称
        # log_dir = os.path.join(log_dir, "eval", dataset_name, 
        #                     os.path.splitext(os.path.basename(args.weight))[0])
        # print(log_dir)
    ...
    if args.task_config:
        ...
        for task_config in all_task_configs:
            ...
            if log_dir:
                # 从路径中提取类似 'FISH_1shot' 的部分
                dataset_name1 = os.path.basename(args.task_config).split('.')[0]  # 例如从 'FISH.yaml' 提取 'FISH'
               
                for idx, dataset_name in enumerate(dataset_names):
                    output_folder = os.path.join(log_dir, os.path.splitext(os.path.basename(args.weight))[0], dataset_name1, "inference")
